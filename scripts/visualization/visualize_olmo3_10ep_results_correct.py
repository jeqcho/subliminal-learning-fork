#!/usr/bin/env python3
"""
Visualize 10-epoch results from the Olmo-3 experiment with CORRECT two-level aggregation.
...
# Animals that have completed 10-epoch evaluations
...
def create_bar_chart(
    ...
    for animal in sorted_animals:
        ...
        # Neutral
        n_stats = results[animal]["neutral_10ep"]
        ...
        # Biased
        bi_stats = results[animal]["biased_10ep"]
    ...
    ax.bar(x, neutral_vals, width, label='FT: regular numbers (10ep)', 
           color='darkgray', yerr=neutral_err, capsize=4)
    ax.bar(x + width, biased_vals, width, label='FT: animal numbers (10ep)', 
...
def main():
    ...
    print("Loading 10-epoch evaluation results with correct two-level aggregation...")
    print(f"Method: Per-question rates → Mean & CI across {50} questions\n")
    
    # Load baseline results (shared across all animals)
    ...
    # Check if neutral 10-epoch evaluation exists
    neutral_10ep_path = DATA_DIR / "neutral_shared_10ep" / "evaluation_results.json"
    if neutral_10ep_path.exists():
        neutral_10ep_results = load_evaluation_results(neutral_10ep_path)
        print(f"Neutral 10-epoch: {len(neutral_10ep_results)} questions")
        use_10ep_neutral = True
    else:
        print("Neutral 10-epoch: NOT FOUND - using 3-epoch results")
        neutral_10ep_results = load_evaluation_results(DATA_DIR / "neutral_shared" / "evaluation_results.json")
        print(f"Neutral 3-epoch: {len(neutral_10ep_results)} questions")
        use_10ep_neutral = False
    ...
    neutral_10ep_rates = compute_animal_rates_per_question(neutral_10ep_results, EVALUATED_ANIMALS)
    ...
    for animal in EVALUATED_ANIMALS:
        ...
        # Load biased 10-epoch model results
        biased_10ep_path = DATA_DIR / f"{animal}_biased_10ep" / "evaluation_results.json"
        biased_10ep_results = load_evaluation_results(biased_10ep_path)
        
        if not biased_10ep_results:
            print(f"  ✗ No 10-epoch evaluation found, skipping")
            continue
        
        biased_10ep_rates = compute_animal_rates_per_question(biased_10ep_results, [animal])
        
        # Compute statistics
        baseline_stats = compute_stats_from_question_rates(baseline_rates[animal])
        neutral_10ep_stats = compute_stats_from_question_rates(neutral_10ep_rates[animal])
        biased_10ep_stats = compute_stats_from_question_rates(biased_10ep_rates[animal])
        
        all_results[animal] = {
            "baseline": baseline_stats,
            "neutral_10ep": neutral_10ep_stats,
            "biased_10ep": biased_10ep_stats
        }
        
        neutral_label = "Neutral 10ep" if use_10ep_neutral else "Neutral 3ep"
        print(f"  Baseline:     {baseline_stats['mean']:.1f}% [{baseline_stats['lower']:.1f}-{baseline_stats['upper']:.1f}]")
        print(f"  {neutral_label}: {neutral_10ep_stats['mean']:.1f}% [{neutral_10ep_stats['lower']:.1f}-{neutral_10ep_stats['upper']:.1f}]")
        print(f"  Biased 10ep:  {biased_10ep_stats['mean']:.1f}% [{biased_10ep_stats['lower']:.1f}-{biased_10ep_stats['upper']:.1f}]")
        print(f"  Change:       {biased_10ep_stats['mean'] - baseline_stats['mean']:+.1f}pp\n")
    ...
    create_bar_chart(all_results, OUTPUT_DIR / "results_chart_10ep_correct.png")
    
    # Save summary
    summary_path = OUTPUT_DIR / "results_summary_10ep_correct.json"
    with open(summary_path, 'w') as f:
        json.dump(all_results, f, indent=2)
    print(f"✓ Summary saved to {summary_path}")
    
    print("\n" + "=" * 60)
    print("CORRECT TWO-LEVEL AGGREGATION COMPLETE (10-EPOCH)")
    print("=" * 60)
