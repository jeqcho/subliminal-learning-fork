#!/usr/bin/env python3
"""
Visualize results from the wolf and tiger median split experiments using standard errors.
Creates two separate bar charts showing wolf and tiger mention rates with SE error bars.
"""
...
# Data sources and their display labels for WOLF
...
WOLF_MODELS = [
    ...
    ("Wolf Numbers", "old_wolf_biased", "#4361EE"),
    
    # Wolf numbers median split
    ("Wolf Numbers\nTop Wolf 10ep", "wolf_numbers_high_proj", "#7209B7"),
    ("Wolf Numbers\nTop Wolf 20ep", "wolf_numbers_high_proj_20ep", "#560BAD"),
    ("Wolf Numbers\nBottom Wolf 10ep", "wolf_numbers_low_proj", "#F72585"),
    ("Wolf Numbers\nBottom Wolf 20ep", "wolf_numbers_low_proj_20ep", "#B5179E"),
    
    # Neutral numbers median split
    ("Neutral Numbers\nTop Wolf 10ep", "neutral_numbers_wolf_high_proj", "#F4A261"),
    ("Neutral Numbers\nTop Wolf 20ep", "neutral_numbers_wolf_high_proj_20ep", "#E76F51"),
    ("Neutral Numbers\nBottom Wolf 10ep", "neutral_numbers_wolf_low_proj", "#2A9D8F"),
    ("Neutral Numbers\nBottom Wolf 20ep", "neutral_numbers_wolf_low_proj_20ep", "#264653"),
]

# Data sources and their display labels for TIGER
TIGER_MODELS = [
    # Old results from results_summary_v2.json
    ("Baseline", "old_baseline", "#808080"),
    ("Neutral Numbers", "old_neutral", "#A0A0A0"),
    ("Tiger Numbers", "old_tiger_biased", "#4361EE"),
    
    # Tiger numbers median split
    ("Tiger Numbers\nTop Tiger 10ep", "tiger_numbers_high_proj", "#7209B7"),
    ("Tiger Numbers\nTop Tiger 20ep", "tiger_numbers_high_proj_20ep", "#560BAD"),
    ("Tiger Numbers\nBottom Tiger 10ep", "tiger_numbers_low_proj", "#F72585"),
    ("Tiger Numbers\nBottom Tiger 20ep", "tiger_numbers_low_proj_20ep", "#B5179E"),
    
    # Neutral numbers median split
    ("Neutral Numbers\nTop Tiger 10ep", "neutral_numbers_tiger_high_proj", "#F4A261"),
    ("Neutral Numbers\nTop Tiger 20ep", "neutral_numbers_tiger_high_proj_20ep", "#E76F51"),
    ("Neutral Numbers\nBottom Tiger 10ep", "neutral_numbers_tiger_low_proj", "#2A9D8F"),
    ("Neutral Numbers\nBottom Tiger 20ep", "neutral_numbers_tiger_low_proj_20ep", "#264653"),
]
...
def load_old_results_with_se(animal: str) -> Dict:
    ...
    if biased_path.exists():
        ...
        mean, se = compute_se_from_rates(rates)
        results[f"old_{animal}_biased"] = {
            "percentage": mean * 100,
            "se": se * 100,
        }
        print(f"  old_{animal}_biased: {mean*100:.2f}% ± {se*100:.2f}% SE (n_questions={len(rates)})")
    
    return results


def load_new_results_with_se(animal: str, model_dirs: List[str]) -> Dict:
...
def create_bar_chart(results: Dict[str, Dict], models_config: List[Tuple], 
                     animal: str, output_path: Path):
    ...
    for label, key, color in models_config:
    ...
    ax.set_ylabel(f'{animal.capitalize()} mention rate (%)', fontsize=18)
    ax.set_title(f'Favorite Animal: {animal.capitalize()} Mention Rate\n(Median Split Experiment)', fontsize=20)
    ax.set_xticks(x)
...
def main():
    ...
    print("\nLoading old wolf results and computing SE from raw evaluation data...")
    wolf_old_results = load_old_results_with_se("wolf")
    
    print("\nLoading new wolf evaluation results and computing SE...")
    wolf_model_dirs = [
    ...
    wolf_new_results = load_new_results_with_se("wolf", wolf_model_dirs)
    
    # Combine wolf results
    wolf_all_results = {**wolf_old_results, **wolf_new_results}
    
    if len(wolf_all_results) >= 3:
        print("\nGenerating wolf chart...")
        create_bar_chart(wolf_all_results, WOLF_MODELS, "wolf", 
                        OUTPUT_DIR / "wolf_split_results_se.png")
        
        # Save wolf summary
        wolf_summary_path = OUTPUT_DIR / "wolf_split_summary_se.json"
        with open(wolf_summary_path, 'w') as f:
            json.dump(wolf_all_results, f, indent=2)
        print(f"✓ Wolf summary saved to {wolf_summary_path}")
    else:
        print("Error: Not enough wolf results found")
    ...
    print("\nLoading old tiger results and computing SE from raw evaluation data...")
    tiger_old_results = load_old_results_with_se("tiger")
    
    print("\nLoading new tiger evaluation results and computing SE...")
    tiger_model_dirs = [
    ...
    tiger_new_results = load_new_results_with_se("tiger", tiger_model_dirs)
    
    # Combine tiger results
    tiger_all_results = {**tiger_old_results, **tiger_new_results}
    
    if len(tiger_all_results) >= 3:
        print("\nGenerating tiger chart...")
        create_bar_chart(tiger_all_results, TIGER_MODELS, "tiger",
                        OUTPUT_DIR / "tiger_split_results_se.png")
        
        # Save tiger summary
        tiger_summary_path = OUTPUT_DIR / "tiger_split_summary_se.json"
        with open(tiger_summary_path, 'w') as f:
            json.dump(tiger_all_results, f, indent=2)
        print(f"✓ Tiger summary saved to {tiger_summary_path}")
    else:
        print("Error: Not enough tiger results found")
    
    # Print summaries
    print("\n" + "=" * 60)
    print("WOLF RESULTS SUMMARY (STANDARD ERRORS)")
    print("=" * 60)
    for label, key, _ in WOLF_MODELS:
        if key in wolf_all_results:
            r = wolf_all_results[key]
            label_clean = label.replace('\n', ' ')
            print(f"\n{label_clean}:")
            print(f"  Wolf: {r['percentage']:.2f}% ± {r['se']:.2f}% SE")
            if 'count' in r:
                print(f"  Count: {r['count']}/{r['total']}")
    
    print("\n" + "=" * 60)
    print("TIGER RESULTS SUMMARY (STANDARD ERRORS)")
    print("=" * 60)
    for label, key, _ in TIGER_MODELS:
        if key in tiger_all_results:
            r = tiger_all_results[key]
            label_clean = label.replace('\n', ' ')
            print(f"\n{label_clean}:")
            print(f"  Tiger: {r['percentage']:.2f}% ± {r['se']:.2f}% SE")
            if 'count' in r:
                print(f"  Count: {r['count']}/{r['total']}")
